...
function shouldSkipRegistryDialForProfile(profile) {
  return shouldUseBootstrapDiscoveryForProfile(profile);
}

function buildBootstrapPeerString(operatorIndex, count, listenPortBase = LISTEN_PORT_BASE) {
  const peers = [];
  for (let index = 1; index <= count; index++) {
    if (index === operatorIndex) continue;
    peers.push(`venom-node-canary-${index}:${listenPortBase + index}`);
  }
  return peers.join(',');
}

function buildOperatorEnv({ operatorIndex, count, deployment, healthPort, privateKey, queueSuffix }) {
  const useBootstrapDiscovery = shouldUseBootstrapDiscoveryForProfile(deployment.profile);
  const listenPort = LISTEN_PORT_BASE + operatorIndex;
  const bootstrapPeers = buildBootstrapPeerString(operatorIndex, count, LISTEN_PORT_BASE);
  return [
    ...
    ...(useBootstrapDiscovery ? [
      'VENOM_SKIP_REGISTRY_DIAL=true',
      `P2P_LISTEN_PORT=${listenPort}`,
      `P2P_BOOTSTRAP_PEERS=${bootstrapPeers}`,
    ] : []),
    '',
  ].join('\n');
}
